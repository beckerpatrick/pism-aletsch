...
RUNLENGTH=0.25
OUTNAME=a${GS}m_low_${PHI_LOW}_high_${PHI_HIGH}-${RATE_FACTOR}_${RUNLENGTH}a_dailyrun.nc
...
echo""
cmd="$PISM_MPIDO $NN $PISM -boot_file $INNAME -skip -skip_max $SKIP $GRID $COUPLER $PHYSICS -extra_file $OUTNAME_EXTRA -extra_times 0:$EXSTEP:$END -extra_vars $EXVARS -o_order zyx  -o $OUTNAME -o_size big -ys $START -ye $END 2>&1 | tee job.\${PBS_JOBID}"
$PISM_DO $cmd
